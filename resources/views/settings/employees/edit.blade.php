...
@section('title', 'Edit Employee - Settings')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-primary card-outline mb-4">
                ...
                <form action="{{ route('settings.employees.update', $employee) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="employee_type" id="employee_type" class="form-select @error('employee_type') is-invalid @enderror" required>
                                        ...
                                        @foreach (App\Enums\EmployeeType::cases() as $type)
                                            <option value="{{ $type->value }}" {{ old('employee_type', $employee->employee_type) == $type->value ? 'selected' : '' }}>
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="specialization_id" id="specialization_id" class="form-select @error('specialization_id') is-invalid @enderror">
                                        ...
                                        @foreach ($specializations as $spec)
                                            <option value="{{ $spec->id }}" {{ old('specialization_id', $employee->specialization_id) == $spec->id ? 'selected' : '' }}>
                                                {{ $spec->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="str_number" id="str_number" class="form-control @error('str_number') is-invalid @enderror" value="{{ old('str_number', $employee->str_number) }}">
                                    @error('str_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input type="checkbox" name="is_active" id="is_active" class="form-check-input @error('is_active') is-invalid @enderror" value="1" {{ old('is_active', $employee->is_active) ? 'checked' : '' }}>
