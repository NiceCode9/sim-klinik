...
@section('title', 'Create Doctor Tariff - Settings')

@section('page-title', 'Create Doctor Tariff')
        ...
        <div class="col-12">
            <div class="card card-primary card-outline mb-4">
                <div class="card-header">
                    <h3 class="card-title">Create New Doctor Tariff</h3>
                </div>
                <form action="{{ route('settings.doctor-tariffs.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="employee_id" id="employee_id" class="form-select @error('employee_id') is-invalid @enderror" required>
                                        ...
                                        @foreach ($doctors as $doctor)
                                            <option value="{{ $doctor->id }}" {{ old('employee_id') == $doctor->id ? 'selected' : '' }}>
                                                {{ $doctor->name }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary">Create Doctor Tariff</button>
